...
## Completed ✓
- [x] [2026-06-06] **New ADPT/Adaptor category** — CarPlay Adapter (Excel row 148). Wired through `generate_products.py` (TYPE_INFO/CAT_URL_SLUG/_CAT_RU), `app.js` (slug maps + RU label), `index.html` (🔌 chip).
- [x] [2026-06-06] **Sold-out products sort last** — stable partition in `app.js render()` pushes `out_of_stock` to the bottom of every view.
- [x] [2026-06-06] **webp duplicate cleanup** — optimizer auto-run now uses `--apply`; deleted 176 leftover originals (~23 MB); 35 double-photo galleries → 0. Build remaps original→webp position-preserving (fixes the photo-order scramble it first caused). Showcase `hof/*.png` → `.webp`.
