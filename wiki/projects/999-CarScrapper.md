...
## Notes & Gotchas
- **Web UI speed depends on a fixed set of indexes + a paginate-first query** — see [[999-CarScrapper-web-perf-indexes]]. If the grid/facets/stats ever feel slow again, first check those indexes exist (`python -m scripts.add_web_indexes`) and that `web/queries.py` still paginates before decorating. The `offer_type`-trailing facet indexes are load-bearing; don't drop them.
...
- **Per-listing timeout must sit BEHIND a concurrency gate, or it times out the queue not the work** (2026-05-30) — `pipeline.py` wraps each worker in `asyncio.wait_for(…, PER_LISTING_TIMEOUT_S=120)` to break the DNS-starvation freeze, but `asyncio.gather` starts *all* coroutines at once while only `CONCURRENT_REQUESTS` (12) can fetch. Without a gate, a listing queued 300-deep burns its whole 120s waiting for a slot → the entire backlog times out in one instant and the `ABORT_AFTER_CONSEC_FAILURES=40` breaker falsely reports a 999.md ban (observed: 113 ok, then 308 timeouts at the same timestamp). Fix: acquire `launch_gate = Semaphore(2 × CONCURRENT_REQUESTS)` *before* starting the wait_for clock, so it measures fetch time. Sized above the site sem so detail+photo phases overlap. Discovery is parallelised (`crawler._discover_parallel`, `DISCOVERY_CONCURRENCY=6`) and the tick timeout is 15 min. These are resilience layers — the throttle root cause (scheduler 12 + refresher 12 = 24 > ~16) is unchanged. See [[../999-CarScrapper-ratelimit-throttle-dns]] + [[../mistakes#2026-05-30 — A per-task asyncio.wait_for timeout over a semaphore-gated gather]].
